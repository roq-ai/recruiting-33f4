...
import { getQualifications } from 'apiSdk/qualifications';
import { QualificationInterface } from 'interfaces/qualification';

import { QualificationListPage as ShowCaseList } from 'pages/qualifications';
...
function DashboardPage() {
  ...
  const jobPostingStat = useSWR<PaginatedInterface<JobPostingInterface>>(
    () => `/job-postings`,
    () => getJobPostings({ limit: 1, offset: 0 }),
  );

  const qualificationStat = useSWR<PaginatedInterface<QualificationInterface>>(
    () => `/qualifications`,
    () => getQualifications({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Job Postings',
      result: jobPostingStat?.data?.totalCount,
      isLoading: jobPostingStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Qualifications',
      result: qualificationStat?.data?.totalCount,
      isLoading: qualificationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`qualifications`} passHref legacyBehavior>
                  <Button
